    ...
    for k in range(len(x)): # for every symbol
        ...
        while True:
            if hi < half: # if lo < hi < 1/2
                ...
                # the half way point. This can be implemented again as 2*(interval - 1/4),
            ...
            lo = lo*2
            hi = hi*2 + 1
            # we break the infinite loop if the interval has reached an un-stretchable state
...
def encode_cont(x, p):
    ...
    for k in range(len(x)):  # for every symbol
        ...
        # Compute cumulative probability as in Shannon-Fano
        pl = dict(sorted([(a, p[k][a]) for a in p[k]], key=lambda el: el[1], reverse=True))

        f = [0]

        for a in pl:
            f.append(np.float64(pl[a] + f[-1]))
        f.pop()

        f = dict([(a, mf) for a, mf in zip(pl, f)])
        ...
        lo = lo + int(ceil(f[a] * lohi_range))
        hi = lo + int(floor(pl[a] * lohi_range))
...
def decode_cont(y, p):
    ...
    x = n * [32]  # initialise all zeros

    for i in range(n):

        pl = dict(sorted([(a, p[i][a]) for a in p[i]], key=lambda el: el[1], reverse=True))
        ...
        if i % 100 == 0:
            print('Arithmetic decoded %d%%    \r' % int(floor(i / n * 100)))
        ...
        # for a = [a for a in f if f[a]<(value-lo)/lohi_range)][-1] for a MUCH slower solution.
        a = bisect(f, (value - lo) / lohi_range) - 1
        new_val = alphabet[a]  # output alphabet[a]
...
def encode_c(x, p, cont_length):
    ...
    for context in pl:
        pl[context] = dict(sorted([(a, pl[context][a]) for a in pl[context]], key=lambda el: el[1], reverse=True))
        f[context] = [0]
        for a in pl[context]:
            f[context].append(np.float64(pl[context][a]) + f[context][-1])
        f[context].pop()

        f[context] = dict([(a, mf) for a, mf in zip(pl[context], f[context])])
    ...
    for k in range(len(x)):  # for every symbol
        ...
        if k < cont_length:
            current_context = str(x[:k])
        else:
            current_context = str(x[k-cont_length:k])
        ...
        lo = lo + int(ceil(f[current_context][a] * lohi_range))
        hi = lo + int(floor(pl[current_context][a] * lohi_range))
...
def decode_c(y, p, n, cont_length):
    """for context method"""
    precision = 64
    one = int(2 ** precision - 1)
    quarter = int(ceil(one / 4))
    ...
    for context in pl:
        ...
        f[context] = [0]
        for a in pl[context]:
            f[context].append(pl[context][a] + f[context][-1])
        f[context].pop()

    for i in range(n):
        if i < cont_length:
            current_context = str(x[:i])
        else:
            current_context = str(x[i-cont_length:i])
        # initialise by taking first 'precision' bits from y and converting to a number
        if i == 0:
            value = int(''.join(str(a) for a in y[0:precision]), 2)
            position = precision  # position where currently reading y
        ...
        alphabet = list(pl[current_context])

        p_current = list(pl[current_context].values())
        ...
        # implements a binary search and is 100 times more efficient. Try
        # for a = [a for a in f if f[a]<(value-lo)/lohi_range)][-1] for a MUCH slower solution.
        a = bisect(f[current_context], (value - lo) / lohi_range) - 1
        new_val = alphabet[a]  # output alphabet[a]

        lo = lo + int(ceil(f[current_context][a] * lohi_range))
        hi = lo + int(floor(p_current[a] * lohi_range))
